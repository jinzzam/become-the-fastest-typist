...
def process_data():
    ...
    if request.method == 'POST':
        ...
        file_to_read = os.path.join("text/" , data + ".txt")
        try:
            with open(file_to_read, "r", encoding="utf-8") as f:
                file_content = f.read()
        except FileNotFoundError:
            # 파일이 없을 경우 
            print("Error:" + file_to_read + "파일을 찾을 수 없습니다.")
        except Exception as e:
            # 기타 파일 읽기 오류 처리
            print(f"파일 읽기 중 오류 발생: {e}")

    # 'templates' 폴더 안에 있는 'type-template.html' 파일을 렌더링하고 내용을 전달
    # render_template('type-template.html', practice_content=file_content)
    return jsonify({'message': '성공적으로 처리되었습니다.', 'content': file_content})
# Flask 앱 실행
if __name__ == '__main__':
    # 디버그 모드는 개발 중에만 사용하고, 프로덕션에서는 비활성화해야 합니다.
    app.run(debug=True)
